...
class BackgroundSubstraction:
    ...
    def compute_masks(self, thresholds, num_contour, pathname, cam_mean, cam_std_dev):
        video = cv.VideoCapture(os.path.dirname(__file__) + pathname)
        masks = []
        ret = True
        while ret:
    ...
    def gridsearch(self, cam_means, cam_std_devs):
        folders = ['cam1', 'cam2', 'cam3', 'cam4']
        thresholds = []
        for i_ in [2, 4, 6, 8, 10]:
            for j_ in [12, 16, 20, 24, 28]:
                for k_ in [12, 16, 20, 24, 28]:
        ...
        for i, f in enumerate(folders):
            ...
            for j in range(len(num_contours)):
                for k in range(len(thresholds)):
                    mask = self.compute_mask(thresholds[k], num_contours[j], '\data\data\\' + f + '\\video.avi',
                                             cam_means[i],
                                             cam_std_devs[i])
    ...
    def background_subtraction(self, thresholds, num_contours, cam_means, cam_std_devs):
        folders = ['cam1', 'cam2', 'cam3', 'cam4']
        camera_masks = []
        for i, f in enumerate(folders):
            masks = self.compute_masks(thresholds[i], num_contours[i], '\data\data\\' + f + '\\video.avi', cam_means[i],
                                       cam_std_devs[i])
